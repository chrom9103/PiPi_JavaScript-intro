...
> 
> 例えば、`console.log()`というコマンドを使うと括弧内の要素をコンソールに出力することができます。  
> これによりプログラムの途中経過や最終結果を確認することができます。
> 
...
> > [!TIP]
> > - コンソールの使い方
> >   1. Visual Studio Codeの画面下部にある「出力」タブをクリックします。
> >   2. プログラムを実行すると、ここに実行結果が表示されます。
> >   3. エラーメッセージが表示された場合は、メッセージを確認してプログラムを修正しましょう。
> 
> コンソールを使いこなすことで、プログラムの動作を効率的に確認し、デバッグを行うことができます。
